...
# MLX90640 IR thermal camera settings
MLX90640_ADDRESS = 0x33
MLX90640_FRAME_RATE = 8.0

# AD7991 ADC settings (4-channel 12-bit ADC)
AD7991_ADDRESS = 0x28  # AD7991-0 variant address
    ...
    def mlx90640_task():
        ...
        while True:
            ...
            if avg_temp is not None:
                ...
                # Toggle frame update flag to signal new data
                if ir_frame_update.value == 0:
                    ir_frame_update.value = 1
                else:
                    ir_frame_update.value = 0
                
                # Copy frame data to shared array
                for i, value in enumerate(frame):
                    ir_frame_array[i] = value
    
    def ad7991_task():
        """Reads all 4 ADC channels at AD7991_TASK_PERIOD rate"""
        start_time = time.time()
        while True:
            current_time = time.time()
            if current_time - start_time > AD7991_TASK_PERIOD:
                # Read all 4 channels from AD7991
                adc_readings = ad7991.read_adc()
                linpot_value.value = adc_readings[0]
                adc1_value.value = adc_readings[1]
                adc2_value.value = adc_readings[2]
                adc3_value.value = adc_readings[3]
...
if __name__ == "__main__":
    ...
    # i2c0 = Hardware I2C bus 0 (for RTC - Real Time Clock)
    # i2c1 = Hardware I2C bus 1 (for MLX90640 IR camera + AD7991 ADC)
    i2c0_handle = SMBus(0)
    i2c1_handle = busio.I2C(board.SCL, board.SDA)

    # Shared values for inter-process communication
    # MLX90640 IR thermal camera data
    avg_temp_value = Value("i", 0)
    ir_frame_array = Array("i", 32 * 24)  # 768 pixels
    ir_frame_update = Value("b", 0)
    
    # AD7991 ADC data (4 channels, 12-bit values)
    linpot_value = Value("i", 0)   # CH0: Linear potentiometer
    adc1_value = Value("i", 0)     # CH1: Analog input 1
    adc2_value = Value("i", 0)     # CH2: Analog input 2
    adc3_value = Value("i", 0)     # CH3: Analog input 3
    ...
    # Create processes
    i2c0_proc = Process(target=i2c0_process, 
                        args=(i2c0_handle,))
    
    i2c1_proc = Process(target=i2c1_process, 
                        args=(i2c1_handle, avg_temp_value, ir_frame_update, ir_frame_array,
                              linpot_value, adc1_value, adc2_value, adc3_value))
